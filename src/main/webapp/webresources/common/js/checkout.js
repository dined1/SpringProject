$( document ).ready(function() {
	...
	$('#BillNumber').on('keyup change', function(){
		var bill_number = '';
		bill_number += $(this).val();
		$('#bill-number').html(bill_number);
	});
	...
	$('#BankCode').on('keyup change', function(){
		var bank_code = '';
		bank_code += $(this).val();
		$('#bank-code').html(bank_code);
	});
	function sendForm() {
		if (!validator.form()) {
		    return false;
	...
	$('input[name=BillNumber]').payment('formatBillNumber');
	$('input[name=cardCVC]').payment('formatCardCVC');
	$('input[name=BankCode]').payment('formatBankCode');
	...
	$.validator.addMethod("BankCode", function(value, element) {
		return this.optional(element) || validateCVC(value);
	}, "Invalid Bank Code.");
	$.validator.addMethod("BillNumber", function(value, element) {
		return this.optional(element) || validateBillNumber(value);
	}, "Invalid Bill Number.");
	...
	paymentFormReady = function() {
		if ($form.find('[name=cardNumber]').hasClass("success") &&
			...
			$form.find('[name=BillNumber]').hasClass("success") &&
			$form.find('[name=BankCode]').hasClass("success") &&
	...
	function validateBillNumber(value){
		if (value.length<16) {
			return false;
		}
		return true;
